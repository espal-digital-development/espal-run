...
func (c *Cockroach) startNodeNonBlocking(storeName string, portNumber int, httpPortNumber int,
	...
	if err := c.stopRunningNodes(storeName, portNumber, "instance"); err != nil {
		return errors.Trace(err)
	}
	if err := c.stopRunningNodes(storeName, httpPortNumber, "http interface"); err != nil {
		return errors.Trace(err)
	}
...
func (c *Cockroach) stopRunningNodes(storeName string, portNumber int, subject string) error {
